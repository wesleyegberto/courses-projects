...
public class SnapshotsResourceIT {
	...
	public JAXRSClientProvider provider = JAXRSClientProvider
			.buildWithURI("http://localhost:8080/monitoring/resources/snapshots");
	...
	public void crud() {
		...
		Response response = provider.target().request().post(Entity.json(payload));
	...
	public void findNotExisting() {
		...
		Response response = provider.target().path(key).request(MediaType.APPLICATION_JSON).get();
